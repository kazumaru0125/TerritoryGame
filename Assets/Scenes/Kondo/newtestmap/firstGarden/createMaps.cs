            ...
            for (int j = 0; j < yokoNumber; j++)
            {
                if (j < tempWords.Length) // ← 安全チェック
                {
                    ...
                    if (testMap[i, j] != null)
                    {
                        ...
                        {
                            case "1"://高い壁
                                Instantiate(hiwalls, new Vector3((-4.5f + j) * scale, 9.8f, (4.5f - i) * scale), Quaternion.identity);
                            ...
                            case "4": // ランタン
                                ...
                                case "11"://新しい壁
                                Instantiate(newhiwalls, new Vector3((-4.5f + j) * scale, 0.0f, (4.5f - i) * scale), Quaternion.Euler(0f, 180f, 0f));
                                break;
