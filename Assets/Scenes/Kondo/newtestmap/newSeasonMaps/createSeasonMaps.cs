...
public class createSeasonMaps: MonoBehaviour
{
    ...
    [SerializeField]
    private GameObject stoneA;
    [SerializeField]
    private GameObject stoneB;
    [SerializeField]
    private GameObject stoneC;
    [SerializeField]
    private GameObject stoneD;
    [SerializeField]
                    ...
                    if (testMap[i, j] != null)
                    {
                        switch (testMap[i, j])
                        {
                            ...
                            case "4": // ランタン
                                ...
                                break;
                                case "7"://アイテムボックス
                                Instantiate(boxs, new Vector3((-4.5f + j) * scale, 0.0f, (4.5f - i) * scale), Quaternion.Euler(0f, 180f, 0f));
                                break;
                                case "8"://ジャンプ
                            ...
                            case "13"://鬼側スタート地点
                                Instantiate(enemyStarts, new Vector3((-4.5f + j) * scale, 0.0f, (4.5f - i) * scale), Quaternion.Euler(0f, 0f, 0f));
                                break;

                            case "21"://全員が超えれる壁
                                Instantiate(stoneA, new Vector3((-4.5f + j) * scale, 0.75f, (4.5f - i) * scale), Quaternion.Euler(0, 0, 0));
