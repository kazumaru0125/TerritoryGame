...
public class AuraChangeScript : MonoBehaviour
    ...
    private TestPlayerRoll tplayerRole;
    ...
    void Start()
        ...
        tplayerRole = GetComponent<TestPlayerRoll>();
        }
    ...
    void Update()
        ...
        if (tplayerRole == null) return;

        if (tplayerRole.CurrentRole == "Human")
            {
            if (!human.isPlaying) human.Play();
            if (oni.isPlaying) oni.Stop();
            }
        else if (tplayerRole.CurrentRole == "Oni")
            {
            if (!oni.isPlaying) oni.Play();
            if (human.isPlaying) human.Stop();
            }
